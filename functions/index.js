...
const liffId = defineSecret('LIFF_ID');
...
exports.config = onRequest({ cors: true, secrets: [liffId] }, (req, res) => {
  res.json({
    LIFF_ID: liffId.value()
  });
});

exports.report = onRequest({ cors: true, secrets: [gmailEmail, gmailPassword, lineLoginChannelId, lineChannelAccessToken, liffId] }, async (req, res) => {
  if (req.method !== 'POST') {
  ...
  try {
    ...
    const loginChannelId = lineLoginChannelId.value();
    ...
    if (validatedData.accessToken) {
      userId = await getUserIdFromAccessToken(validatedData.accessToken, loginChannelId);
    ...
    if (userId && channelAccessToken) {
      lineResult = await sendLineMessage(userId, validatedData, saveResult, channelAccessToken);
    }
...
function validateAndSanitizeData(rawData) {
  ...
  if (!rawData.photoDataDistant || !rawData.photoDataClose) {
    throw new Error('遠景写真および近景写真の両方の投稿が必要です。');
  }
...
async function getUserIdFromAccessToken(accessToken, loginChannelId) {
  try {
    // トークン検証
    const verifyResponse = await axios.get(`${LINE_VERIFY_TOKEN_URL}?access_token=${accessToken}`);
    if (verifyResponse.data.client_id !== loginChannelId) {
...
async function sendLineMessage(userId, reportData, saveResult, channelAccessToken) {
  try {
    const messages = [];
